...
export default function ShiftRegisterModal({baseYear, baseMonth, isOpen, setIsOpen}: ShiftRegisterModalProps) {
    return (
        <>
            {isOpen &&
                <ShiftRegisterModalContent key={`${baseYear}-${baseMonth}`} baseYear={baseYear} baseMonth={baseMonth}
                                           setIsOpen={setIsOpen}/>}
        </>
    )
}

function ShiftRegisterModalContent({baseYear, baseMonth, setIsOpen}: Omit<ShiftRegisterModalProps, "isOpen">) {
    ...
    const {mutate: uploadScheduleMutate, isPending} = useMutation({
        ...
        onError: (error) => {
            ...
            window.alert('근무 스케줄 업로드에 실패하였습니다.');
        }
    })
    ...
    return (
        <div className="fixed inset-0 z-30 flex items-end justify-center bg-foreground/35 px-3 pb-14.5"
            ...
            <section role="dialog" aria-modal="true" aria-labelledby="add-schedule-title"
                ...
                <button type="button" disabled={!uploadedFile || isPending} onClick={uploadSchedule}
